...
- **E6.7** · _E6 · Multi-user platform_ — **Security review - GATE before inviting anyone** _(depends E6.6, web)_ · [spec](features/E6_database_design.md)
  ...
  - STILL OPEN, so THE GATE REMAINS CLOSED: (1) custom SMTP - measured this session, mailer_autoconfirm is false and disable_signup is false, so a stranger can start signing up and can never finish; (2) the Auth redirect allowlist still sends confirmation and password-reset links to http://localhost:3000 - probed with an invalid token, our prod URL falls back exactly like a bogus control URL; (3) pb-proxy.portfoliobuilder.workers.dev still answers 200 - a SECOND unauthenticated public Yahoo proxy that nothing uses and that doubles the ToS exposure; needs the owner's go-ahead to delete; (4) no size cap on a stored portfolio - accepted, since E10.1 bounds history at 10 versions.
